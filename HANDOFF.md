...
- GitHub Actions [运行 #35835524120](https://github.com/Liutupi/nabo-tab5/actions/runs/35835524120) 已验证两个变体均可在 ESP-IDF 6.1 下完整编译，并检查 Rev 1.x 固件确实链接专用 ISP 库。
...
2. 已烧录提交 `a56d47a` 的 CI 产物并确认启动至配网模式，详见第 8 节。
3. 请人工验证实际显示、触摸坐标、背光、扬声器/双麦、拍照画质、连接路由器与休眠唤醒。
4. 排查 SC202CS 默认 IPA 配置中的色彩矩阵越界：低色温矩阵含 `4.5445`，超出 ESP-IDF ISP 的 `[-4, 4]` 范围；目前拍照自检仍能抓帧，但会产生报错且可能影响颜色。优先做针对性的矩阵限幅/配置修正，并以真机样张验证，不要只屏蔽日志。
...
- SC202CS 自检抓帧期间，默认 IPA 的 CCM 色彩矩阵会触发数百条越界错误；相机初始化成功不等于画质验收完成。
...
| 固件 | `a56d47a70e00709f124d717f2aa32350de9e88fb`，`nabo-tab5`，合并固件 SHA-256 `d8ccfee0e28e1de5cff2ba0ac7b1e8633dcebc630bcdb846fadb02db23019b24` |
| 烧录 | `/dev/cu.usbmodem212401`，写入后哈希校验通过 |
| 屏幕/触摸 | 检出 ST7121 面板、720×1280 显示驱动和 ST7123 触摸控制器（最多 10 点）；实际视觉/触摸操作待人工确认 |
| 摄像头 | 检出 SC202CS `PID=0xeb52`；自检 5 秒抓取 150 帧，原先 `Illegal instruction` 不再发生；存在 CCM 越界报错 |
| 音频 | 双工通道创建、音频编解码器启动；音质/双麦待人工确认 |
| C6/网络 | SDIO 初始化成功，进入 `wifi_configuring`，热点及配置 Web 服务启动；路由器连接待测，且有 Host/C6 版本不一致警告 |
| 稳定性 | 复位后观察约 20 秒，无 Guru Meditation 或自动重启；更长时间运行待测 |
