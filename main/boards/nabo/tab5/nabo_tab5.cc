...
public:
    Pi4ioe1(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr) {
        ...
        // LCD_RST (P4) must be input-pull-up while released. Driving it high
        // as an output can disturb the shared I2C bus on ST7121/ST7123 units.
    ...
    void SetLcdResetAsserted(bool asserted) {
        ...
        if (asserted) {
            // Program the low level before enabling push-pull output.
            clrbit(output, 4);
            WriteReg(PI4IO_REG_OUT_SET, output);
            setbit(direction, 4);
            WriteReg(PI4IO_REG_IO_DIR, direction);
        } else {
            // Release reset as input with pull-up, per M5Stack's ST712x note.
...
private:
    ...
    void ResetLcdAndTouch() {
        ...
        pi4ioe1_->SetLcdResetAsserted(true);  // P4 = LCD_RST
        clrbit(value, 4);
...
public:
    NaboTab5Board() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeI2c();
        I2cDetect();
        InitializePi4ioe();
